...
if options.admin_acct and options.admin_passwd:
    for schema in db_schemas:
        Popen(['sqlplus','-S',options.admin_acct+'/'+options.admin_passwd+'@'+options.database,'<','tmp_'+schema])
        Popen(['rm','tmp_'+schema])
    for acct,passwd in [(options.reader_acct,options.reader_passwd),(options.writer_acct,options.writer_passwd)]
        if acct and passwd:
            Popen(['sqlplus','-S',acct+'/'+passwd+'@'+options.database,'<','synoyms.sql'])
